	...
	internal class BaseVisualThing : VisualThing, IVisualEventReceiver
	{
		...
		public bool Setup()
		{
			...
			if(Thing.Sector != null)
			{
                ...
                {
                    if (Thing.IsFlagSet("32"))//BlendMask1
                    {
                        renderstyle = "translucent";
                        alpha = (byte)(General.Clamp(0.5f, 0f, 1f) * 255);
                    }
                    else if (Thing.IsFlagSet("64"))//BlendMask2
                    {
                        renderstyle = "add";
                        alpha = (byte)(General.Clamp(1.0f, 0f, 1f) * 255);
                    }
                    else if (Thing.IsFlagSet("128"))//BlendMask3
                    {
                        renderstyle = "subtract";
                        alpha = (byte)(General.Clamp(1.0f, 0f, 1f) * 255);
                    }

                    if ((Thing.IsFlagSet("32") && Thing.IsFlagSet("64") && Thing.IsFlagSet("128")) ||//BlendMask1 + BlendMask2 + BlendMask3
                        (Thing.IsFlagSet("32") && Thing.IsFlagSet("64")))//BlendMask1 + BlendMask2
                    {
                        //renderstyle = "translucent";
                        renderstyle = "add";
                        alpha = (byte)(General.Clamp(0.25f, 0f, 1f) * 255);
                    }
                    else if ((Thing.IsFlagSet("32") && Thing.IsFlagSet("128")))//BlendMask1 + BlendMask3
                    {
                        renderstyle = "subtract";
                        alpha = (byte)(General.Clamp(1.0f, 0f, 1f) * 255);
                    }
                ...
                switch (renderstyle)
				{
                    ...
                    case "subtract": //[GEC]
                        RenderPass = RenderPass.Subtractive;
                        break;
			...
			if(Thing.Sector != null)
			{
				...
				if(!info.Bright)
				{
					...
					if(level != null)
					{
                        ...
                        if (General.Map.PSXDOOM)//[GEC]
                        {
                            fogfactor = VisualGeometry.CalculateFogFactor(level.sector, 255);
                            base.lightmode = 3;
                        }
                    }
				}
