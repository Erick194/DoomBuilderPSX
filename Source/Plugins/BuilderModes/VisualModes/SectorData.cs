...
namespace CodeImp.DoomBuilder.BuilderModes
{
	internal class SectorData
	{
		...
		private void BasicSetup()
		{
            ...
            int sector_Brightness = sector.Brightness;

            if (General.Map.PSXDOOM)//[GEC]
            {
                sector_Brightness = 255;
            }
            ...
            if (!lightfloorabsolute) lightfloor = sector_Brightness + lightfloor;
            if (!lightceilingabsolute) lightceiling = sector_Brightness + lightceiling;
			...
			ceilingbase.brightnessbelow = sector_Brightness;
			ceilingbase.color = PixelColor.FromInt(mode.CalculateBrightness(sector_Brightness)).WithAlpha(255).ToInt();
