...
{
    internal class PSXDoomMapSetIO : MapSetIO
    {
        #region ================== Constants
        ...
        // Constructor
        public PSXDoomMapSetIO(WAD wad, MapManager manager) : base(wad, manager)
        {
        }
        ...
        private Dictionary<int, Vertex> ReadVertices(MapSet map, int firstindex)
        {
            ...
            int num = (int)lump.Stream.Length / 8;
            BinaryReader reader = new BinaryReader(mem);
        ...
        private Dictionary<int, Sector> ReadSectors(MapSet map, int firstindex)
        {
            ...
            int num = (int)lump.Stream.Length / 28;
            BinaryReader reader = new BinaryReader(mem);
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                {
                    int fnum;
                    if (int.TryParse(flag.Key, out fnum)) stringflags[flag.Key] = ((flags & fnum) == fnum);
                }
        ...
        private void WriteSectors(MapSet map, int position, Dictionary<string, MapLumpInfo> maplumps)
        {
            ...
            foreach (Sector s in map.Sectors)
            {
                ...
                writer.Write((Byte)s.IdxColor);//[GEC]
