...
namespace CodeImp.DoomBuilder.Data
{
	public sealed class DataManager
	{
        ...
        /*internal*/
        public Bitmap GetTextureBitmap(string name) { Vector2D unused = new Vector2D(); return GetTextureBitmap(name, out unused); }
        /*internal*/
        public Bitmap GetTextureBitmap(string name, out Vector2D scale)
		{
        ...
        //public static void UpdateSkybox() { if (skybox2 != null) skybox2.Dispose(); skybox2 = null; }
        internal void SetupSkybox()
		{
			...
			if(skybox == null)
			{
				...
				if(General.Map.Graphics.CheckAvailability())
				{
                    ...
                    Vector2D scale = new Vector2D(1.0f, 1.0f);
                    Bitmap bmp = tex.GetBitmap();
                    if (General.Map.PSXDOOM && General.Map.Map.Sectors.Count != 0)//[GEC]
                    {
                        string Skyname = null;
                        foreach (Sector s1 in General.Map.Map.Sectors)
                        {
                            if (s1.CeilTexture.Contains("F_SKY"))//remove f_sky
                            {
                                Skyname = s1.CeilTexture.Substring(2, s1.CeilTexture.Length - 2);
                            }

							if (!General.Map.PSXDOOM_IFST)
							{
								if (s1.FloorTexture.Contains("F_SKY"))//remove f_sky
								{
									Skyname = s1.FloorTexture.Substring(2, s1.FloorTexture.Length - 2);
								}
							}
						}
                        ...
                        if (Skyname != null)
                        {
                            Bitmap bmp2 = GetTextureBitmap(Skyname, out scale);
        ...
        public static CubeTexture MakeClassicSkyBox2(Bitmap img) { return MakeClassicSkyBox(img, new Vector2D(1.0f, 1.0f)); }
